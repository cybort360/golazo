...
function FeaturedHero({ pool }: { pool: SponsoredPool }) {
  return (
    <div className="relative overflow-hidden rounded-3xl bg-ink p-7 text-white">
      <div className="pointer-events-none absolute -bottom-12 -right-8 select-none text-[200px] font-black opacity-[0.06]">🏆</div>
      <div className="relative max-w-2xl">
        ...
        <div className="mt-2 flex items-center gap-2 text-[18px] font-extrabold text-neon">🏆 {pool.prize}</div>
        ...
        <div className="mt-5 flex items-center gap-5">
          {pool.joined ? (
            <span className="inline-flex items-center gap-1.5 rounded-full bg-green-600 px-5 py-2.5 text-[14px] font-extrabold text-white">
              ✓ Joined{pool.yourRank ? ` · ranked #${pool.yourRank}` : ""}
            </span>
          ) : (
  ...
  return (
    <div className="hidden lg:block">
      ...
      <div className="bg-ink px-8 py-7 text-white">
        <div className="flex items-end justify-between gap-6">
          <div>
            ...
            <div className="mt-1 text-[28px] font-black tracking-[-0.03em]">Prize pools 🏆</div>
      ...
      <div className="px-8 py-8">
        ...
        {/* compliance note */}
        <div className="mt-4 rounded-xl border border-dashed border-slate-200 bg-white px-4 py-3 text-[12px] font-semibold text-slate-500">
          🛡️ Pools are always free to enter. Prizes are merch, access &amp; perks — never cash or real-money wagering.
        </div>
